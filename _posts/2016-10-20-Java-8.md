...
**Java 8'de neler yeni?**  
Adam L. Davis

**Preface**

Bir çok Java geliştiricisi gibi lambda ifadelerini ilk duyduğumda ilgimi çekmişlerdi. Aynı zamanda geciktirileceğini duymak birçokları gibi beni de hayak kırıklığına uğratmıştı. Yine de hiç olmamasındansa geç olması iyidir.

Java 8, Java dili için dev bir ileri adım. Bu kitabı yazmak[1] beni bu konuda daha fazla şey öğrenmeye zorladı. Project Lambda ile, Java yeni bir closure söz dizimi(syntax), method-referansları ve arayüzlerde(interface) öntanımlı methodlara sahip oldu. Fonksiyonel dillerin sahip olduğu bir çok özellik, dilin yalınlığı ve sadeliği kaybedilmeden eklendi.

Project Lambda dışında, Java 8 ile birlikte yeni bir Date ve Time API (JSR 310), Nashorn JavaScript motoru(engine) geldi ve bir çok başka değişikliğin yanında Permanent Generation, HotSpot sanal makinesinden(virtual machine) kaldırıldı.
